...
def save_raw_llm_output(text, output_dir, filename):
    """Persist the raw LLM response for later debugging."""
    if output_dir is None:
        return
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, filename)
    with open(output_path, "w") as f:
        f.write("" if text is None else text)
    ...
    while generated_text is not None:
        ...
        if PATH_TO_LATEST_OUTPUT_JSON == "":
            ...
            assert tool_call["name"] in {
                "segment_phrase",
                "report_no_mask",
            }, f"Unexpected first tool call: {tool_call['name']}"
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                segmentation_result = segmentation_tool.segment_phrase(
                    image_path=img_path,
                    text_prompt=current_text_prompt,
                    output_dir=sam_output_dir,
                    verbose=verbose,
                )
                PATH_TO_LATEST_OUTPUT_JSON = segmentation_result.json_path
                sam3_outputs = segmentation_result.data
            ...
            for i in range(num_masks):
                ...
                mask_check_count += 1
                save_raw_llm_output(
                    checking_generated_text,
                    llm_log_dir,
                    f"raw_mask_check_{mask_check_count:03d}_mask_{i + 1:03d}.txt",
                )
